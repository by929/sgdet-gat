...
def generate_filenames(mode, start_i):
	...
	if mode == 'test':
		...
		filenames.append('vg_test_26000-26446_feat_relabel.txt')
	elif mode == 'train':
		for i in range(start_i, 56000, 1000):
			# filename = 'vg_{}_{}-{}_feat_relabel2.txt'.format(mode, i, i+1000)
			filename = 'vg_{}_{}-{}_feat_relabel.txt'.format(mode, i, i+1000)
			filenames.append(filename)
		filenames.append('vg_train_56000-56224_feat_relabel.txt')
	return filenames

def generate_edge_batch(boxes, boxes_start_id, edge_file, box_tok_file, topk):
	boxes_dis = dis_arr(boxes, len(boxes))

	boxes_topk = boxes_dis.argsort(1)
	for i in range(boxes_topk.shape[0]):
		box_tok_file.write(str(i+boxes_start_id))
		for j in boxes_topk[i][1:]:
			box_tok_file.write(' ' + str(j+boxes_start_id))
		box_tok_file.write('\n')
...
>>>>>>> origin/master
	...
	# raw_data_filenames = ['vg_{}_0-1000_feat_relabel.txt'.format(mode)]
	...
	for filename in raw_data_filenames:
		...
		for line in raw_data:
			if new_file:	# 每100张图存成1个文件
				feat_filename = 'vg_{}_{}-{}_gat_feat.txt'.format(mode, img_id, img_id + img_sava_num)
				feat_path = "./data/{}/{}_gat_feat".format(mode, mode)
				feat_file = open(os.path.join(feat_path, feat_filename), 'w')

				edge_filename = 'vg_{}_{}-{}_gat_edge_top{}.txt'.format(mode, img_id, img_id + img_sava_num, topk-1)
				edge_path = "./data/{}/{}_gat_edge_top{}".format(mode, mode, topk-1)
				edge_file = open(os.path.join(edge_path, edge_filename), 'w')

				box_tok_filename = 'vg_{}_{}-{}_box_topk.txt'.format(mode, img_id, img_id + img_sava_num)
				box_tok_path = "./data/{}/{}_box_topk".format(mode, mode)
				box_tok_file = open(os.path.join(box_tok_path, box_tok_filename), 'w')

				new_file = 0

			data = line.strip().split(' ')
			if float(data[0]) != img_id:
				generate_edge_batch(img_boxes, boxes_start_id, edge_file, box_tok_file, topk)
				generate_node_batch(img_nodes, feat_file)
				img_id += 1
				if img_id % img_sava_num == 0:
					new_file = 1
				img_boxes.clear()
				img_nodes.clear()
